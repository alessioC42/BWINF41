...
bestLenght = 999999999

def step(path:list[P], points:list[P], endLen:int, cb, counter):
    global bestLenght, vis
    if (len(path) == endLen):
        counter(vis.generateLineSVG(path))
        lenght = getTotalLenght(path)
        if lenght < bestLenght:
            bestLenght = lenght
            cb(vis.generateLineSVG(path), lenght)
        return -1
    ...
    for point in first(5, distancemap):
        newPath = path.copy()
        newpoints = points.copy()
        newPath += [point[1]]
        newpoints.remove(point[1])
        step(newPath, newpoints, endLen, cb, counter)

def getStartPoint(points):
    p = min(points, key=lambda x: x.x)
    return p
